...
class ExpenseService:
    ...
    def get_expenses(self, searchDto: ExpenseSearchDto):
        ...
        return ExpenseService.ExpenseFilter.apply(
            expenses,
            min_amount=1,
    ...
    def save_expenses(self, searchDto: ExpenseSearchDto):
        raw_data = self.get_raw_expenses(searchDto)
        expenses = self.expense_transformer.transform(json.loads(raw_data))
        filter_expenses = ExpenseService.ExpenseFilter.apply(
            expenses,
            min_amount=1,
            start_date=searchDto.start_date,
            end_date=searchDto.end_date,
    ...
    class ExpenseFilter:
        ...
        def apply(expenses, min_amount=0, start_date=None, end_date=None):
            return [
                ...
                and ExpenseService.ExpenseFilter._is_in_date_range(
                    exp.usage_date, start_date, end_date
                )
            ]
